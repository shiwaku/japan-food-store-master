...
> → issue #46 / [japan-food-access-analysis#3](https://github.com/shiwaku/japan-food-access-analysis/pull/3)。**投入の是非は 47県・地方部限定で測り直すまで確定していない。**
...
2. ~~**fresh_food の補完**~~ → **2026-08-27 に許可データで実測し、ATP 基準に投入済み**
   ...
   **⚠ ただしこれは3県での数字で、47県では投入後のほうが悪い**（issue #46）。生鮮単独の47県での効果は未測定。
...
3. **許可データ⑪の投入 → ATP 基準には投入済み**（issue #26、`docs/sources/検証_許可データ_総合スーパー_補完効果.md`）。
   ...
   **⚠ この「純増が都市部に多い」性質が47県では致命的に効く**（比>1 が倍増・issue #46）。
