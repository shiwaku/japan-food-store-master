...
- **許可データで cat を業種コードから振ってはいけない**（2026-09-21・`docs/sources/検証_JFF単独レイヤ_店舗レイヤ適性.md`）。
  コンビニは店内調理があるため **`① 飲食店営業` で届け出ている**のが普通で、
  `⑩ コンビニエンスストア` だけで拾うと大手3社の網羅率が **28.5%** にしかならない。
  店名でチェーン照合すれば **66.7%** まで上がる。`⑪ 百貨店、総合スーパー` も
  supermarket 49.9% / drugstore 34.1% の混在。**店名によるチェーン判定が主、業種は従**。
  なお `data/facilities-all.csv` の出所は [Japan Food Facilities](https://github.com/gl20percentclub/japan-food-facilities)
  （`https://food.japan-facilities.com/api/facilities-all.csv`・毎週更新）。
  **このソース単独では店舗レイヤにならない**（47県・地方部で r 0.2472→0.2144、圏外率 56.3%→66.9%）。
