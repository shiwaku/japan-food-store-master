...
| 用途 | ファイル | 件数 |
|---|---|---:|
| **推計** | `data/food_store_master_atp_super.parquet` | **122,249** |
| **公開**（viewer・配布物） | `data/food_store_master.parquet`（Phase1） | **102,984** |

`atp_based`（119,092）/ `atp_fresh`（136,335）/ `atp_permit`（139,492）は
「許可データを入れるとどうなるか」を測るために作った**途中生成物で、結論が出た今は使わない**
（`merge_permit_gapfill.py` でいつでも再生成できる）。経緯は
`docs/sources/検証_許可データ_47県での再評価.md`。

### 採否は「③ 操作化」で判定する（2026-08-28 確定）

農水省定義への合致には2層あり、**両方を満たす系統は存在しない**。判定は③で行う。

| 層 | 測り方 | 最良の系統 |
|---|---|---|
| ② 店舗レイヤの定義＝業態の列挙 | カテゴリ別の件数を実数統計と比べる | ATP＋生鮮（平均乖離 19.9pt） |
| **③ 操作化＝同一500mメッシュ内の店舗存否** | **比>1 の件数と農水省公表値との相関 r** | **ATP＋⑪（r 0.199 / 比>1 6件）** |

**件数（②）が合っていても空間分布が合っていなければ農水省の推計は再現できない**ので、
③を優先する。実際、②で最良の ATP＋生鮮は③では最悪（r 0.153 / 比>1 16件）＝
生鮮の純増が都市部と離島に偏っているため。**②の件数比は参考値として見る**（下の
加重カバー率の注意書きと同じ理由）。

| 系統 | 件数 | ② 平均乖離 | ③ 比>1 | ③ r（地方部） |
|---|---:|---:|---:|---:|
| **ATP＋⑪** | 122,249 | 42.6pt | **6** | **0.199** |
| ATP基準 | 119,092 | 39.2pt | 6 | 0.200 |
| Phase1 | 102,984 | 49.1pt | 5 | 0.191 |
| ATP＋生鮮 | 136,335 | **19.9pt** | **16** | **0.153** |
| ATP＋両方 | 139,492 | 25.9pt | 16 | 0.154 |

> ⚠️ ②の平均乖離は **supermarket 行が歪んでいる**（issue #33）。分子が広義
> （Overture supermarket ＋ grocery_store）で分母がセンサス581 なので 132〜146% と出るが、
> 分子を `src_cat='supermarket'` に絞り分母を 561+581 にすると **79.0%＝不足**。
> 層②の数字を単独で根拠にしないこと。

設計と結果は `docs/master/設計_ATP基準マスター構築.md`。
**公開マスター（Phase1）は変更していない。**

> ### ⚠️ 3県だけで測った数字は残っているが、判定には使わない（2026-08-27・issue #46）
>
> 経緯として: 3県（宮城・島根・高知）では ATP 基準で drugstore が 42.5%→111.6%、
> 500m圏外率 62.2%→59.8%、許可データ 20,400件を足して 57.3% / r 0.448→0.462 と
> すべて改善して見えた。**1,740市区町村で測り直すと逆転する**:
>
