# 食品営業許可オープンデータ 再現パイプライン（**検証用**）

> ⚠️ **これは検証用のパイプラインで、食料品店マスターの構築系統ではない。**
> ここで再現したデータは `data/food_store_master.*` に**投入していない**
> （マスターは Overture と OSM の2ソースのみ。`src` 列は overture 97,600 / osm 5,384 の2値）。
> マスターを作りたいなら `scripts/build_food_store_master.py` を見ること。
> 位置ソースに採らなかった理由は [`docs/permits/README.md`](../../docs/permits/README.md)。
