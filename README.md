...
> **生鮮（fresh_food）の穴は許可データで埋まる見込み**（2026-08-27）。生鮮3業種（魚介類・食肉・
> 野菜果物 販売業）からの純増 21,511店で、3県の500m圏外率 62.2%→60.1%、農水省公表値との相関も改善（r 0.448→0.460）。
...
加えて 2026-08-26 に **ATP 基準マスター**（119,092 店）を並行系統として追加した。チェーン公式サイト由来の
...
| 食品営業許可オープンデータ | **検証**＋**穴埋めの投入候補**（未投入。⑪総合スーパーで純増4,191件、生鮮3業種で純増21,511件を実測） | 各自治体（保健所）／ **厚生労働省**「食品衛生申請等システム（FAS）」 | 各データのオープンライセンス |
