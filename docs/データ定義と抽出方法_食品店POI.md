...
- 元データ = **2015年推計:『平成26年全国消費実態調査』の個票 ＋『民力』の市町村別自動車登録台数（軽自動車含む）＋ 国勢調査（人口・世帯数）**／**2020年推計:『家計調査』個票等**（全国消費実態調査の全国家計構造調査への改組に伴い差替）。
- 参考値（全国・2015年）: 65〜74歳の自動車利用率 79.7%／75歳以上 57.0%。都道府県差大。

##### 「自動車利用困難率」の具体的な算出（薬師寺2017・2018で確認）

2つの率を出して中間を取る。

1. **自動車所有率**（個人が車を持つ割合）: 全国消費実態調査の個票から、性×年齢18区分の「1人あたり車台数」を**定数項なし重回帰**で推定→登録台数に校正→都道府県値を「自動車普及率（登録台数÷世帯数）＋単身世帯比率」の回帰で市区町村へダウンスケール→**RAS調整**。
2. **自動車利用可能率**（車のある世帯に属する世帯員の割合。本人非所有でも同居家族が持てば可）: 手順は所有率とほぼ同じ（登録台数への校正はしない）。
3. **自動車利用困難率 = { (1−所有率) + (1−利用可能率) } ÷ 2**（両者の中間値）
   - 非所有率だけだと家族の車を使える人まで困難に数え**過大**、非利用可能率だけだと**過小**なので折衷。
   - **実例（2015年・65歳以上）**: 所有率44.4%（非所有55.6%）、利用可能率68.8%（非利用可能31.2%）→ 困難率 =(55.6+31.2)/2= **43.4%**。

- **民力（登録台数）の役割**: 比例配分ではなく、「自動車普及率＝登録台数÷世帯数」を回帰の説明変数＋RASの制約に使い、都道府県の率を市区町村へダウンスケールするため。首都圏・近畿圏の低所有はダミー変数で吸収。
- 年齢区分（65-74歳/75歳以上）は按分ではなく、性×年齢区分に最初から別カテゴリとして保持。
- **旧方式（2017年より前・初期のアクセスマップ）**: 都道府県別「自動車普及率」＋2003年住宅・土地統計調査の自動車所有世帯割合から関係式（`log(自動車非所有率)=-0.05216-1.2916×自動車普及率`, R²=0.9396）を作り市町村へ適用。65歳以上/75歳以上は内閣府世論調査の利用率格差で按分。世帯/個人単位の曖昧さと2003年以降データ不足のため2017年方式へ刷新。
- **⚠️ なお未確認**: 2020年推計が都道府県別粒度を維持したかは報告会PDFに明記なし。「自動車利用困難率＝中間値」の定義は薬師寺2018講演スライド（出典表記=薬師寺2017）で確認。
...
- **出典 = 国勢調査 地域メッシュ統計（500mメッシュ）の65歳以上人口**。2020年推計＝令和2年国勢調査。75歳以上も同メッシュ統計から分離集計。
- **人口そのものは推計値ではなく国勢調査の実測値**。推計されるのは「困難」の部分（店舗距離の確率 × 自動車利用困難割合）のみ。
- メッシュごとの算出手順:
  1. 人口のある500mメッシュごとに、**当該＋周辺（3×3）メッシュの店舗分布から「最寄り店舗が500m以上である確率」を算出**（半径500mの円による確率法。「メッシュ内に店舗が無い＝困難」というハードなカットではない）
  2. この確率に国勢調査の65歳以上人口を乗じ「店舗まで500m以上の人口」を出す
  3. さらに `(1 − 自動車利用率)` を掛けて自動車利用困難分を反映
- **⚠️ 一次資料（240319_01.pdf）で未確認**: ①65〜74歳/75歳以上の2区分を500mメッシュで直接取得か按分か、②500mメッシュでの年齢5歳階級の提供有無、③小メッシュ人口の秘匿処理。高橋(2018)論文本文での追確認が必要。

> 総括すると **困難人口 ≒ 国勢調査65歳以上人口（実測） × P(最寄り店舗≥500m)（確率法） × (1 − 自動車利用率)（統計按分）**。3要素のうち人口だけが実測、店舗距離は確率、自動車は統計按分。
...
- **薬師寺哲郎(2017)『高齢者の自動車利用状況の推計』食料供給プロジェクト【食料品アクセス】研究資料 第3号 第7章, pp.87-113**（自動車利用率の推計式の一次資料・本文PDF取得済み）: https://www.maff.go.jp/primaff/kanko/project/attach/pdf/170331_29kyokyu3_07.pdf
- 研究資料第3号 目次（全文一括版あり）: https://www.maff.go.jp/primaff/kanko/project/29kyokyu3.html
- 薬師寺(2018)講演スライド「高齢者の自動車利用」（「自動車利用困難率＝中間値」の定義を含む）: https://www.maff.go.jp/primaff/koho/seminar/2018/attach/pdf/180704_03.pdf
